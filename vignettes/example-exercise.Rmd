...
We suppress or show the solutions with the knitr global `include` chunk option controlled by a setup chunk at the beginning of the document as follows
...
This function sets `knitr::opts_chunk(include)`. If knitting with the _Knit_ button, when `solutions:` is set to `TRUE` it adds the suffix `-solutions` to the output filename before the file extension. And when `solutions:` is set to `FALSE` it adds the suffix `-questions` to the output filename before the file extension.
